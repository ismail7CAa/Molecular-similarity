...
def _pairwise_smiles_features(smiles_a: str, smiles_b: str) -> dict[str, float]:
    ...
    return {
        ...
        "atom_token_jaccard": float(
            _jaccard_similarity(set(atom_tokens_a), set(atom_tokens_b))
        ),
...
def _rdkit_pairwise_features(smiles_a: str, smiles_b: str) -> dict[str, float]:
    ...
    return {
        "morgan_tanimoto_radius2": float(
            DataStructs.TanimotoSimilarity(morgan_a_r2, morgan_b_r2)
        ),
        "morgan_dice_radius2": float(DataStructs.DiceSimilarity(morgan_a_r2, morgan_b_r2)),
        "morgan_tanimoto_radius3": float(
            DataStructs.TanimotoSimilarity(morgan_a_r3, morgan_b_r3)
        ),
...
def _atom_tokens(smiles: str) -> list[str]:
    ...
    while index < len(smiles):
        ...
        if character.isalpha() and (character.isupper() or character in {"c", "n", "o", "s", "p"}):
...
def categorical_feature_names(rows: list[dict[str, object]]) -> list[str]:
    ...
    names.extend(
        f"standard_type={name}"
        for name in sorted({str(row["standard_type"]) for row in rows})
    )
    return names


def vectorize_sql_row(
    row: dict[str, object],
    ...
    base_row = dict(row)
    values = vectorize_row(base_row, numeric_feature_names, [], numeric_stats)
    for feature_name in categorical_names:
        key, expected = feature_name.split("=", maxsplit=1)
        values.append(1.0 if str(base_row[key]) == expected else 0.0)
    return values
...
def choose_best_configuration(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    for feature_set_name, numeric_feature_names in FEATURE_SET_CANDIDATES.items():
        for use_categories in (False, True):
            ...
            train_features = [
                vectorize_sql_row(row, numeric_feature_names, active_categories, numeric_stats)
            ...
            for l2_penalty in L2_CANDIDATES:
                ...
                val_probabilities = [
                    float(probability)
                    for probability in model.predict_proba(val_features)[:, 1]
                ]
...
def build_report(export_path: Path) -> dict[str, object]:
    ...
    numeric_stats = compute_numeric_stats(split_to_rows["train"] + split_to_rows["val"], numeric_feature_names)

    development_rows = split_to_rows["train"] + split_to_rows["val"]
    development_features = [
        vectorize_sql_row(row, numeric_feature_names, category_names, numeric_stats)
    ...
    development_probabilities = [
        float(probability)
        for probability in model.predict_proba(development_features)[:, 1]
    ]
    test_probabilities = [
    ...
    return {
        ...
        "test_examples": [
            ...
            for row, probability in zip(split_to_rows["test"][:20], test_probabilities[:20], strict=True)
        ],
    }
...
def main() -> int:
    export_path = Path(sys.argv[1]).expanduser().resolve() if len(sys.argv) == 2 else DEFAULT_EXPORT_PATH
